...
describe("For multiple styles in component", () => {
  ...
  const componentStyle = result.find(
    (item) => item.rootName === "componentStyle"
  );
...
describe("findStylesUsed", () => {
  test("Case 1", () => {
    ...
    const containerStyleIsUsed = stylesUsed.find(
      (item) => item.name === "container"
    );
    const textStyleIsUsed = stylesUsed.find((item) => item.name === "text");
    const rootStyle = stylesUsed.every(
      (item) => item.rootStyleName === "styles"
    );
...
const file4 = () => {
  return (
    ...
    expect(
      stylesUsed.find((item) => item.rootStyleName === "componentStyle")
    ).toBeDefined();
    expect(
      stylesUsed.find((item) => item.name === "componentContainer")
    ).toBeDefined();
    expect(
      stylesUsed.find((item) => item.rootStyleName === "styles")
    ).toBeDefined();
...
describe("getStyleContents", () => {
  ...
  test("nested style", () => {
    ...
    const styleForPasting = getStyleContents(selection);
    expect(styleForPasting).toEqual([
      "flex: 1",
